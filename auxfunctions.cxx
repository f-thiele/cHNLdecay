...
Double_t int__i(Double_t* x, Double_t* par) {
   ...
   Double_t xd = par[1];
   Double_t xl = par[2];
   // Double_t _s = (TMath::Power(xx, 2)+TMath::Power(xl, 4)+TMath::Power(xd, 4)-2*xx*xl*xl-2*xx*xd*xd-2*xl*xl*xd*xd)*(TMath::Power(1, 2)+TMath::Power(xx, 2)+TMath::Power(xu, 4)-2*xx-2*xu*xu-2*xx*xu*xu);
   Double_t f = (xx-xl*xl-xd*xd)/xx*(1+xu*xu-xx)*TMath::Sqrt(kaellen(xx, TMath::Power(xl, 2), TMath::Power(xd, 2))*kaellen(1, xx, TMath::Power(xu, 2)));
...
TGraph* create_graph(Double_t xd, Double_t xl, Float_t low, Float_t high, Float_t stepsize) {
  ...
  for(Double_t x = low; x <= high; x=x+stepsize) {
    ...
    if(TMath::IsNaN(_temp)) continue;
    Double_t _temp0 = I_xu_xd_xl(x, 0, 0);
    if(TMath::IsNaN(_temp0)) continue;
  ...
  for(UInt_t i = 0; i<res_y.size(); i++) {
    ...
    if(i > 0 && (ratio-res_y.at(i-1))/ratio>0.5) continue;
